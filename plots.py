...
def contour_theta(frequency, x_axis, distance, anisotropy_rotation_y, rotation_z, reflectivities):
    ...
    fig.suptitle("ATR for $\phi_y$ = " + str(int(round(np.degrees(anisotropy_rotation_y),1))) + "$^\circ$, $\phi_z$ = " + str(int(round(np.degrees(rotation_z),1))) + "$^\circ$, $d = " + str(round(distance * 1e4, 3)) +"\mu m$")
...
def contour_azimuth(frequency, rotations, distance, anisotropy_rotation_y, rotation_z, reflectivities):
    ...
    for data, title, row, col in ax_to_plot:
        im = ax[row, col].pcolormesh(rotations, frequency, data.T, cmap='magma')
        cbar = plt.colorbar(im, ax=ax[row, col])
        cbar.mappable.set_clim(0., 1.)
        cbar.set_label(title)
        ax[row, col].set_title(title)
        ax[row, col].set_xlabel('Azimuth rotation (radians)')
...
def contour_y_anisotropy(frequency, x_axis, distance, incident_angle, rotation_z, reflectivities):
