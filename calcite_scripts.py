...
def main_incident_one_axis_anisotropy():
    ...
    quartz_layer = berreman_incidence_one_anisotropy(
        kx, eps_tensor, non_magnetic_tensor, semi_infinite=True
    )
...
def main_all_anisotropy_axes():
    ...
    calcite = CalciteUpper(frequency_length=150, run_on_device_decorator=run_on_device)

    k0 = calcite.frequency * 2.0 * m.pi
    eps_tensor = calcite.fetch_permittivity_tensor()

    air_gap_thickness = tf.cast(tf.linspace(0.0e-4, 3.0e-4, 30), dtype=tf.complex64)
    ...
    z_rotation = tf.cast(tf.linspace(0.0, m.pi / 2.0, 15), dtype=tf.complex64)
    ...
    calcite_layer = berreman_all_anisotropy(
        kx,
        eps_tensor,
    ...
    T = prism_layer @ air_layer @ calcite_layer
    del calcite_layer, air_layer, prism_layer, kx, eps_tensor, non_magnetic_tensor
