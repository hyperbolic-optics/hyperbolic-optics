...
def main_all_anisotropy_axes():
    ...
    eps_prism = 5.5
    incident_angle = tf.linspace(-tf.constant(m.pi, dtype=tf.float32) / 2, tf.constant(m.pi, dtype=tf.float32) / 2, 20)
    kx = tf.cast(tf.sqrt(eps_prism) * tf.sin(incident_angle), dtype = tf.complex64)
    distance = 1.5e-4

    quartz = Quartz(frequency_length=40, run_on_device_decorator=run_on_device)
    
    k0 = quartz.frequency * 2. * m.pi
    eps_tensor = quartz.fetch_permittivity_tensor()

    x_rotation = tf.cast(tf.linspace(0.,m.pi/2.,10), dtype = tf.complex64)
    y_rotation = tf.cast(tf.linspace(0.,m.pi/2.,45), dtype = tf.complex64)
    z_rotation = tf.cast(tf.linspace(0.,2 * m.pi,65), dtype = tf.complex64)
    ...
    air_layer = (tf.linalg.inv(berreman_incidence(kx, non_magnetic_tensor, non_magnetic_tensor, k0, thickness = distance)))[:,:,tf.newaxis, tf.newaxis, tf.newaxis, :, :]
