...
def main_all_anisotropy_axes():
    ...
    eps_prism = 5.5
    incident_angle = tf.linspace(-tf.constant(m.pi, dtype=tf.float32) / 2, tf.constant(m.pi, dtype=tf.float32) / 2, 100)
    kx = tf.cast(tf.sqrt(eps_prism) * tf.sin(incident_angle), dtype = tf.complex64)
    distance = 1.5e-4
    ...
    z_rotation = tf.cast(tf.linspace(0.,2 * np.pi,65), dtype = tf.complex64)
    ...
    air_layer = (tf.linalg.inv(berreman_incidence_one_anisotropy(kx, non_magnetic_tensor, non_magnetic_tensor, k0, thickness = distance)))[:,:,tf.newaxis, tf.newaxis, tf.newaxis, :, :]
