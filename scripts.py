...
from plots import (all_axis_plot, azimuthal_slider_plot, contour_plot_simple_incidence, 
                   plot_dispersion_semi_infinite, plot_one_dispersion, plot_determinant,
                   plot_rpp, spatial_plot)
...
def main_propagation(material_type):
    ...
    material = material_type(frequency_length=50, run_on_device_decorator=run_on_device)

    k_0 = material.frequency * 2.0 * m.pi
    eps_tensor = material.fetch_permittivity_tensor()

    air_gap_thickness = tf.cast(tf.linspace(0.0e-4, 1.5e-4, 120), dtype=tf.complex64)
    x_rotation = tf.cast(tf.linspace(0.0, m.pi/2., 2), dtype=tf.complex64)
    y_rotation = tf.cast(tf.linspace(0., m.pi/2., 5), dtype=tf.complex64)
    z_rotation = tf.cast(tf.linspace(0.0, 2. * m.pi, 90), dtype=tf.complex64)
    ...
    print(reflectivity_values.shape)
...
def main_dispersion(material_type):
    ...
    x_rotation_1 = tf.constant(0.)
    y_rotation_1 = tf.constant(0.)
    z_rotation_1 = tf.constant(m.pi/4.)

    eps_tensor = anisotropy_rotation_one_value(
        eps_tensor, x_rotation_1, y_rotation_1, z_rotation_1
    )
    ...
    # plot_dispersion_semi_infinite(material, mode_1, mode_2, incident_angle.numpy().real)
    plot_one_dispersion(material, layer, k_x.numpy().real)


def main_rpp_analysis(material_type):
    """
    Computing the transfer matrix for a semi infinite anisotropic material,
      for all possible anisotropy rotations.
    """
    ...
    air_gap_thickness = tf.cast(tf.linspace(0.0e-4, 1.5e-4, 2), dtype=tf.complex64)
    x_rotation = tf.cast(tf.linspace(0.0, m.pi/2., 2), dtype=tf.complex64)
    y_rotation = tf.cast(tf.linspace(0.0, m.radians(23.3), 3), dtype=tf.complex64)
    z_rotation = tf.cast(tf.linspace(m.radians(0.), m.radians(360.), 180), dtype=tf.complex64)
    ...
    # Reflection Coefficient
    reflectivity_values = reflection_coefficients(transfer_matrix)[0,0,:,:,0,1,:]
    # reflectivity_values = tf.transpose(reflectivity_values, perm=[0,2,1])

    # print(reflectivity_values.shape)

    plot_rpp(reflectivity_values.numpy(), material, incident_angle.numpy().real, z_rotation.numpy().real, eps_prism)

if __name__ == "__main__":
    # main_dispersion(Quartz)
    # main_rpp_analysis(Quartz)
