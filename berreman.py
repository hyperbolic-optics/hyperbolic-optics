...
def berreman_one_angle(
    ...
    element13 = mu_tensor[..., 1, 0] - (
        mu_tensor[..., 1, 2] * mu_tensor[..., 2, 0] / mu_tensor[..., 2, 2]
    )
    element14 = (
        mu_tensor[..., 1, 1]
    ...
    element31 = (
        eps_tensor[..., 1, 2] * eps_tensor[..., 2, 0] / eps_tensor[..., 2, 2]
    ) - eps_tensor[..., 1, 0]
    element32 = (
        (kx**2) / mu_tensor[..., 2, 2]
        - eps_tensor[..., 1, 1]
        + (eps_tensor[..., 1, 2] * eps_tensor[..., 2, 1] / eps_tensor[..., 2, 2])
    )
    ...
    element41 = eps_tensor[..., 0, 0] - (
        eps_tensor[..., 0, 2] * eps_tensor[..., 2, 0] / eps_tensor[..., 2, 2]
    )
    element42 = eps_tensor[..., 0, 1] - (
        eps_tensor[..., 0, 2] * eps_tensor[..., 2, 1] / eps_tensor[..., 2, 2]
    )
    ...
    if semi_infinite:
        ...
        ordered_eigenvectors = tf.gather(
            eigenvectors, sorted_indices, axis=-1, batch_dims=1
        )
...
def berreman_incidence_one_anisotropy(
    kx,
    eps_tensor,
    mu_tensor,
    k0=None,
    ...
    if semi_infinite:
        ...
        ordered_eigenvalues = tf.gather(
            eigenvalues, sorted_indices, axis=-1, batch_dims=3
        )
        ...
        # Reorder eigenvectors using sorted_indices
        ordered_eigenvectors = tf.gather(
            eigenvectors, sorted_indices, axis=-1, batch_dims=3
        )
