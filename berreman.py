...
def main_incident():
    ...
    eps_prism = 5.5
    incident_angle = tf.linspace(-tf.constant(m.pi, dtype=tf.float32) / 2, tf.constant(m.pi, dtype=tf.float32) / 2, 2000)
    
    kx = tf.cast(tf.sqrt(eps_prism) * tf.sin(incident_angle), dtype = tf.complex128)

    quartz = Quartz(frequency_length=2000, run_on_device_decorator=run_on_device)
    ...
    kx = tf.expand_dims(kx, axis=1)
    eps_tensor = tf.expand_dims(eps_tensor, axis=0)
    non_magnetic_tensor = tf.expand_dims(non_magnetic_tensor, axis=0)

    quartz_layer = layer_matrix_incidence_tensorflow(kx, eps_tensor, non_magnetic_tensor, semi_infinite=True)


    T = tf.matmul(prism_layer, quartz_layer)
    r = reflection_coefficients(T)

    contour_plot('theta',r,  quartz.frequency.numpy().real, incident_angle.numpy().real, 0., None, rotation_x = 0., rotation_y = 0., rotation_z = 0.)
