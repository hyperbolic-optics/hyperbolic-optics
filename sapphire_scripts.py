...
from material_params import Sapphire, Ambient_Incident_Prism, Air
...
from berreman import reflection_coefficients, berreman_all_anisotropy, berreman_incidence
...
def main_all_anisotropy_axes():
    ...
    eps_prism = 5.5
    incident_angle = tf.linspace(-tf.constant(0, dtype=tf.float32) / 2, tf.constant(m.pi, dtype=tf.float32) / 2, 30)
    ...
    y_rotation = tf.cast(tf.linspace(0.,m.pi/2.,9), dtype = tf.complex64)
    ...
    sapphire_layer = berreman_all_anisotropy(kx, eps_tensor, non_magnetic_tensor * tf.ones_like(eps_tensor), semi_infinite=True)[tf.newaxis, ...]
    ...
    azimuthal_slider_plot(r.numpy(), incident_angle.numpy().real, sapphire, x_rotation.numpy().real, y_rotation.numpy().real, z_rotation.numpy().real, air_gap_thickness.numpy())
